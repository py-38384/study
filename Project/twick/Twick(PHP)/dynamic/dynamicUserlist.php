...
if (mysqli_num_rows($sql) == 1) {
    ...
    $sql = mysqli_query($connect,"SELECT * FROM broedcast ORDER BY msg_id DESC LIMIT 1");
    if(mysqli_num_rows($sql)){
        $row = mysqli_fetch_assoc($sql);
        $broedcast_msg = "";
        if($row["msg_sender_id"] == $_SESSION["unique_id"]){
            if(strlen($row["msg"])<= 11){
                $broedcast_msg = "You: ".substr($row["msg"], 0, 11);
            }else{
                $broedcast_msg = "You: ".substr($row["msg"], 0, 11).". . .";
            }
        }else{
            ...
            if(strlen($row["msg"])<= 15){
                $broedcast_msg = $sender_row["firstname"]." ".$sender_row["lastname"].": ".substr($row["msg"], 0, 15);
            }else{
                $broedcast_msg =  $sender_row["firstname"]." ".$sender_row["lastname"].": ".substr($row["msg"], 0, 15).". . .";
            }
        }
        $output.= '
    <a href="../twick/broedcast.php"> 
    <div class="content">
        <div class="details">
            <strong>Broedcast Room</strong>
            <p>'.$broedcast_msg.'</p>
        </div>
    </div>
</a>
